...
#By looping through each alternate name 
    ...
    #Define a regex pattern that would match all of the name variants
...
#Add the place if not in the dictionary yet 
        ...
        if month_key not in place_count[place]:
            #If not add it with a count of zero 
            place_count[place][month_key] = 0

        #Add the current count to the total for that month   
        place_count[place][month_key] += count
...
for place in place_count:
    ...
    month_list = list(place_count[place].keys())

    #Print how many times the place was mentioned in each month by looping through it 
    for month in month_list:
        count = place_count[place][month] #Exact count for that month 
...
for place in place_count:
    ...
    for month in place_count[place]:
        count = place_count[place][month]
